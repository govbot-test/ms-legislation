...
Formatted, versioned legislative data for Mississippi. Part of the [govbot](https://github.com/chihacknight/govbot) civic data project.
...
This is the **formatted data repo** for Mississippi — one of two repos per jurisdiction:

1. **Scraper repo** (`govbot-openstates-scrapers/ms-legislation`) — runs [OpenStates](https://github.com/openstates/openstates-scrapers) scrapers, produces raw scraped JSON
2. **This repo** (`ms-legislation`) — reads the scraper repo's output and:
...
To change Mississippi's configuration (branch pinning, runner, enabled jobs, labels), edit its entry in [`chn-openstates-files.yml`](https://github.com/chihacknight/govbot/blob/main/actions/pipeline-manager/chn-openstates-files.yml) and re-run `apply.py` — don't hand-edit the workflow files in this repo directly, since the next `apply.py` run will overwrite them.

Current settings for Mississippi:

- **State code**: `ms`
- **Toolkit branch**: `main`
- **Scraper repo**: `govbot-openstates-scrapers/ms-legislation`
...
Pulls the latest scraped data from the scraper repo, sanitizes it, formats it into `country:us/state:ms/sessions/...`, links events, and updates orphan tracking.
...
├── country:us/
│   └── state:ms/            # state:usa for federal, state:il for Illinois, etc.
│       └── sessions/
│           └── {session_id}/
│               ├── bills/
│               │   └── {bill_id}/
...
Formatted metadata is saved to `country:us/state:ms/sessions/`, organized by session and bill.
...
```json
{
  "HB999": {
    ...
    "vote_count": 2,
    "event_count": 0,
    "path": "country:us/state:ms/sessions/103/bills/HB999"
  }
}
```
...
- Check the scraper repo (`govbot-openstates-scrapers/ms-legislation`) actually got fresh data first — this repo only formats what that repo has already scraped
